...
DramaCoo uses a **single orchestrator + multi-stage agents** design: one orchestrator drives a state machine, dispatching dedicated agents across 6 stages, each persisting artifacts and waiting for user confirmation.
...
```bash
git clone https://github.com/Innate-Labs/DramaCoo.git
cd DramaCoo
cd .
chmod +x install.sh
./install.sh
...
Backend config lives in `backend/config.yaml` (lowercase, hierarchical YAML). Edit it directly or via the frontend **Settings** page.
...
All metadata and generated assets are stored under `backend/code/`:
...
DramaCoo is evolving toward true multi-agent collaboration:
