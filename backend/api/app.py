...
DEFAULT_CORS_ORIGINS = ["http://127.0.0.1:3000", "http://localhost:3000", "http://127.0.0.1:3002", "http://localhost:3002"]
...
async def lifespan(app: FastAPI):
    logger.info("Starting DramaCoo API")
    logger.info("Code directory mounted at /code: %s", settings.CODE_DIR)
    from core.db import init_db

    init_db()
    yield
    logger.info("DramaCoo API shutdown complete")


app = FastAPI(title="DramaCoo", version="2.0.0", lifespan=lifespan)
...
@app.get("/")
async def root():
    return {"service": "DramaCoo", "version": "2.0.0", "health": "/api/health"}
