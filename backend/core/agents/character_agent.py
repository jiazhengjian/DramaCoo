...
class CharacterDesignerAgent(AgentInterface):
    ...
    def _list_versions(self, sid: str, asset_type: str, asset_id: str) -> List[str]:
        ...
        # 排除缩略图文件（_thumb.webp 不是历史版本）
        files = [f for f in files if "_thumb." not in os.path.basename(f)]
    ...
    def _build_preview(self, sid: str, chars_desc: dict, sets_desc: dict, props_desc: dict = None) -> dict:
        """构建素材预览列表（含当前状态）用于前端实时显示"""
        preview = {"characters": [], "settings": [], "props": []}
        ...
        for prop_id, info in (props_desc or {}).items():
            existing = self._list_versions(sid, 'props', prop_id)
    ...
    def _generate_one(self, img_client, asset_id: str, name: str, desc: str,
        ...
        from core.tasks import create_task_for_generation
        from core.thumbnail import make_image_thumb
        item_type = "characters" if asset_type == "characters" else "settings"
        task_id, task_store = create_task_for_generation(
            sid, type="image", stage="character_design", item_type=item_type,
            item_id=asset_id, model=t2i_model, prompt=base_prompt,
        )
        ...
        for iteration in range(max_iterations):
            try:
                ...
                task_store.mark_failed(task_id, "已取消")
                raise
            ...
            try:
                ...
                if is_acceptable:
                    # 评估通过，返回结果
                    make_image_thumb(save_path)
                    task_store.mark_completed(task_id, [save_path])
                    ...
                    # 报告进度
                    self._report_progress("资产图", f"重新生成中 ({iteration + 2}/{max_iterations}): {name}", 0)
        ...
        if len(all_versions) > 1:
            ...
            if best_path:
                ...
                make_image_thumb(best_path)
                task_store.mark_completed(task_id, [best_path])
                return asset_id, best_path, best_eval

        # 没有多个版本或 VLM 选择失败，返回最后一次结果
        final_path = save_path if os.path.exists(save_path) else None
        if final_path:
            make_image_thumb(final_path)
            task_store.mark_completed(task_id, [final_path])
        else:
            task_store.mark_failed(task_id, "生成失败：所有迭代均未产出有效图片")
        return asset_id, final_path, eval_result if 'eval_result' in locals() else None
    ...
    def _read_script_data(self, input_data: Dict) -> dict:
        """从编排器注入的 script_generation artifact 提取角色、场景、道具数据。"""
        ...
        props = {}
        for p in script_gen.get("props", []) or []:
            pid = p.get("prop_id") or p.get("id") or ""
            if pid:
                props[pid] = {
                    "name": p.get("name", ""),
                    "type": p.get("type", ""),
        ...
        return {"characters": chars, "settings": sets, "props": props}

    # ─── 道具图生成 ───

    @staticmethod
    def _prop_prompt(name: str, prop_type: str, desc: str, style_prompt: str) -> str:
        template = load_prompt('prop', 'prop', 'zh')
        return template.format(name=name, type=prop_type, description=desc, style=style_prompt)

    def _generate_prop(self, img_client, prop_id: str, name: str, prop_type: str,
                       desc: str, style: str, t2i_model: str, sid: str) -> tuple:
        """生成单个道具白底单品图，返回 (prop_id, path_or_None)。不参与 VLM 评估。"""
        from core.tasks import create_task_for_generation
        from core.thumbnail import make_image_thumb

        style_prompt = self._get_style_prompt(style)
        prompt = self._prop_prompt(name, prop_type, desc, style_prompt)
        save_path = self._next_version_path(sid, 'props', prop_id)
        save_dir = os.path.dirname(save_path)

        task_id, task_store = create_task_for_generation(
            sid, type="image", stage="character_design", item_type="props",
            item_id=prop_id, model=t2i_model, prompt=prompt,
        )
        try:
            paths = img_client.generate_image(
                prompt=prompt, model=t2i_model,
                session_id=str(sid), save_dir=save_dir, video_ratio="1:1", resolution="2K",
            )
            if not paths:
                task_store.mark_failed(task_id, "生成失败：无返回图片")
                return prop_id, None
            gen = paths[0]
            if gen != save_path:
                if os.path.exists(save_path):
            ...
            make_image_thumb(save_path)
            task_store.mark_completed(task_id, [save_path])
            return prop_id, save_path
        except Exception as e:
            logger.error(f"Prop gen failed for {prop_id}: {e}")
    ...
    async def process(self, input_data: Any, intervention: Optional[Dict] = None) -> Dict:
        ...
        if intervention:
            ...
            props_desc = {}
            input_props = input_data.get("props", [])
            if isinstance(input_props, list):
                for p in input_props:
                    if isinstance(p, dict) and (p.get("id") or p.get("prop_id")):
                        pid = p.get("id") or p.get("prop_id")
                        props_desc[pid] = {"name": p.get("name", ""), "type": p.get("type", ""), "description": p.get("description", "")}
            elif isinstance(input_props, dict):
                props_desc = input_props
            ...
            if regen_chars or regen_sets:
                self._report_progress("资产图", "重新生成中...", 10)
                tasks = []
                ...
                def regen_run():
                    ...
                    with ThreadPoolExecutor(max_workers=concurrency) as executor:
                        ...
                        for atype, aid, name, desc, species in tasks:
                            ...
                            self._report_progress("资产图", f"正在生成: {name}", 10, data={
                                "asset_complete": {
                                    "type": atype,
                                    "id": aid,
                                    "status": "running",
                        ...
                        for fut in as_completed(futs):
                            ...
                            if result_path:
                                ...
                                self._report_progress("资产图", f"完成: {fname}", pct, data={
                                    "asset_complete": {
                                ...
                                self._report_progress("资产图", f"失败: {fname}", pct, data={
                                    "asset_complete": {
            ...
            self._report_progress("资产图", "完成", 100)
            return self._build_payload(sid, chars_desc, sets_desc, props_desc, select_chars, select_sets)

        # ═══════════ 正常流程: 全量首次生成 ═══════════
        self._report_progress("资产图", "读取剧本数据...", 5)

        # 优先从 input_data 获取素材定义，如果没有再从原始剧本文件读取
        chars_desc, sets_desc, props_desc = {}, {}, {}
        if input_data and isinstance(input_data, dict):
            ...
            props_raw = input_data.get("props")
        ...
        # 发送素材预览（含所有素材和当前状态）
        preview = self._build_preview(sid, chars_desc, sets_desc, props_desc)
        self._report_progress("资产图", "加载素材列表", 8, data={"assets_preview": preview})
        ...
        def run():
            ...
            for prop_id, info in props_desc.items():
                existing = self._list_versions(sid, 'props', prop_id)
                if existing:
                    continue
                all_tasks.append(("props", prop_id, info.get("name", ""), info.get("description", ""), info.get("type", "")))

            if not all_tasks:
                self._report_progress("资产图", "所有素材已存在", 95)
                return
            ...
            with ThreadPoolExecutor(max_workers=concurrency) as executor:
                futs = {}
                for atype, aid, name, desc, extra in all_tasks:
                    existing_versions = self._list_versions(sid, atype, aid)
                    self._report_progress("资产图", f"正在生成: {name}", 10, data={
                        "asset_complete": {
                            "type": atype,
                    ...
                    if atype == "props":
                        fut = executor.submit(
                            self._generate_prop, img_client,
                            aid, name, extra, desc, style, t2i_model, sid,
                        )
                    else:
                        fut = executor.submit(
                            self._generate_one, img_client,
                            aid, name, desc, atype, style, extra, t2i_model, vlm_model, sid,
                        )
                    futs[fut] = (atype, aid, name)
        ...
        return self._build_payload(sid, chars_desc, sets_desc, props_desc)

    def _build_payload(self, sid: str, chars_desc: dict, sets_desc: dict, props_desc: dict = None,
                       selected_chars: dict = None, selected_sets: dict = None, selected_props: dict = None) -> dict:
        """构建返回给前端的 payload"""
        ...
        for prop_id, info in (props_desc or {}).items():
            ...
            sel = selected_props.get(prop_id, "")
            prop_info = self._build_asset_info(sid, 'props', prop_id, name, desc, sel)
            prop_info["type"] = ptype
