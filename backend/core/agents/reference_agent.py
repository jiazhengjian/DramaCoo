...
class ReferenceGeneratorAgent(AgentInterface):
    ...
    def _generate_one(self, img_client, sid: str, segment: dict,
        ...
        task_id, task_store = create_task_for_generation(
            sid, type="image", stage="reference_generation", item_type="scenes",
            item_id=segment_id, model=model, prompt=first_frame_prompt,
        )
        ...
        for version in range(max_versions):
            ...
            try:
                ...
                if version < max_versions - 1:
                    ...
                    self._report_progress("分镜图", f"重新生成中 ({version + 2}/{max_versions}): {segment_id}", 0)
        ...
        if all_versions:
            ...
            if best_path and isinstance(best_eval, dict):
                ...
                task_store.mark_failed(task_id, "VLM选择的最佳图仍有硬性失败项")
        ...
        task_store.mark_failed(task_id, "生成失败：未成功生成任何参考图")
        return segment_id, None, None
    ...
    async def process(self, input_data: Any, intervention: Optional[Dict] = None) -> Dict:
        ...
        if intervention:
            ...
            if regen_scenes:
                self._report_progress("分镜图", "重新生成中...", 2)
                ...
                def regen_run():
                    ...
                    def regen_segment_run(segment_id: str, index: int):
                        ...
                        self._report_progress("分镜图", f"正在生成: {segment_id}", 5, data={
                            "asset_complete": {
                                "type": "scenes",
                                "id": segment_id,
                                "status": "running",
                    ...
                    # 并发生成提示词与图像
                    self._report_progress("分镜图", f"生成参考图... 0/{total}", 5)
                    with ThreadPoolExecutor(max_workers=concurrency) as executor:
                        ...
                        for fut in as_completed(futs):
                            ...
                            if result_path:
                                ...
                                self._report_progress("分镜图", f"完成: {segment_id_done}", pct, data={
                                    "asset_complete": {
                                ...
                                self._report_progress("分镜图", f"失败: {segment_id_done}", pct, data={
                                    "asset_complete": {
                                        "type": "scenes", "id": segment_id_done,
                                        "status": "failed",
                                        "selected": "", "versions": [],
        ...
        def run():
            ...
            if not pending_segments:
                self._report_progress("分镜图", "所有分镜图已存在", 95)
                return
            ...
            self._report_progress("分镜图", f"开始生成... 0/{total}", calc_pct(0))
            ...
            with ThreadPoolExecutor(max_workers=concurrency) as executor:
                ...
                def segment_run(seg: dict, index: int):
                    ...
                    self._report_progress("分镜图", f"正在生成: {segment_id}", calc_pct(done), data={
                        "asset_complete": {
            ...
            if cancelled:
                self._report_progress("分镜图", "已取消（保留已完成图片）", 96)
            else:
                self._report_progress("分镜图", "保存结果...", 96)
        ...
        try:
            ...
            if "cancel" in str(e).lower():
                ...
                self._report_progress("分镜图", "已取消（保留已完成图片）", 100)
