...
class VideoDirectorAgent(AgentInterface):
    ...
    def _generate_one(self, sid: str, segment_id: str, prompt: str,
        ...
        from core.thumbnail import extract_video_poster
        ...
        task_id, task_store = create_task_for_generation(
            sid, type="video", stage="video_generation", item_type="clips",
            item_id=segment_id, model=video_model, prompt=prompt,
            params={"duration": duration, "mode": video_generation_mode},
        )
        ...
        try:
            ...
            poster = extract_video_poster(save_path)
            task_store.mark_completed(task_id, [save_path] + ([poster] if poster else []))
    ...
    @classmethod
    def _build_mention_map(cls, character_art: dict) -> dict[str, str]:
        """构建 @名字 → 选中参考图路径 的映射（角色/场景/道具）。"""
        mapping: dict[str, str] = {}
        for key in ("characters", "settings", "props"):
            for asset in (character_art.get(key) or []):
                if not isinstance(asset, dict):
                    continue
                name = str(asset.get("name") or "").strip()
                path = cls._asset_selected_path(asset)
                if name and path:
                    mapping[name] = path
        return mapping
    ...
    async def process(self, input_data: Any, intervention: Optional[Dict] = None) -> Dict:
        ...
        if intervention:
            ...
            if regen_ids:
                self._report_progress("分镜视频", "重新生成中...", 5)
                ...
                def regen_run():
                    ...
                    with ThreadPoolExecutor(max_workers=concurrency) as executor:
                        ...
                        for seg_id in regen_ids:
                            ...
                            self._report_progress("分镜视频", f"启动生成: {seg_id}", 5, data={
                                "asset_complete": {
                        ...
                        for fut in as_completed(futs):
                            ...
                            if res_path:
                                ...
                                self._report_progress("分镜视频", f"完成: {sid_done}", pct, data={
                                    "asset_complete": {
                                ...
                                self._report_progress("分镜视频", f"失败: {sid_done}", pct, data={
                                    "asset_complete": {
                                        "type": "clips", "id": sid_done,
                                        "status": "failed",
                                        "selected": "", "versions": [],
        ...
        # ═══ 正常流程：全量生成 ═══
        self._report_progress("分镜视频", "正在准备数据...", 2)
        preview = self._build_preview(sid, segments, scene_map, video_clips)
        self._report_progress("分镜视频", "加载视频列表", 5, data={"assets_preview": {"clips": preview}})

        def run():
            tasks = []
            from core.mentions import resolve_mentions

            mention_map = self._build_mention_map(character_art)
            for seg_index, seg in enumerate(segments):
                ...
                if video_generation_mode == "start_end_frame" and not last_img_path:
                    logger.warning("VideoDirectorAgent: %s 首尾帧模式缺少尾帧，回退为首帧生视频入参", seg_id)
                # @提及：把 prompt 中的 @角色名/@场景名/@道具名 对应参考图加入
                if mention_map:
                    prompt, mention_paths = resolve_mentions(prompt, mention_map)
                    if mention_paths:
                        reference_image_paths = list(reference_image_paths or []) + mention_paths
                tasks.append((seg_id, prompt, img_path, duration, last_img_path, reference_image_paths))
            if not tasks:
                self._report_progress("分镜视频", "所有视频片段已存在", 95)
                return
            done = 0
            with ThreadPoolExecutor(max_workers=concurrency) as executor:
                futs = {}
        ...
        self._report_progress("分镜视频", "完成", 100)
        return self._build_payload(sid, segments, video_clips)
