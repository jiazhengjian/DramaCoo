...
export default function WorkflowPanel() {
  ...
  useEffect(() => {
    // Stage changes also occur during automatic generation, without a route change.
    workflowRef.current?.closest('.xyq-main')?.scrollTo({ top: 0, left: 0, behavior: 'instant' });
  }, [sessionId, activeStage]);

  const activeStageStatus = activeStage ? stageStates[activeStage]?.status : undefined;
  useEffect(() => {
    const workflow = workflowRef.current;
    const main = workflow?.closest<HTMLElement>('.xyq-main');
    if (!workflow || !main || !activeStage) return;
    const header = workflow.querySelector<HTMLElement>('.xyq-topbar');
    const footer = workflow.querySelector<HTMLElement>('.xyq-stage-actions');
    const updateInsets = () => {
      main.style.setProperty('--layout-sticky-header', `${header?.offsetHeight || 0}px`);
      main.style.setProperty('--layout-sticky-footer', `${footer?.offsetHeight || 0}px`);
    };
    updateInsets();
    ...
    return () => {
      observer.disconnect();
      main.style.removeProperty('--layout-sticky-header');
      main.style.removeProperty('--layout-sticky-footer');
    };
  }, [activeStage, activeStageStatus]);
  ...
  const handleRetryFailed = async (stageId: string) => {
    if (!sessionId) return;
    try {
      const res = await retryFailedTasks(sessionId, stageId);
      if (res.retried > 0 && !isRunning) {
        updateStageState(stageId, {
          status: 'running',
          progress: 0,
          progressMessage: `重试 ${res.retried} 个失败项...`,
          artifact: null,
          error: null,
        });
        pollForCompletion(sessionId, stageId);
      }
    } catch (error: any) {
  ...
  const renderStageContent = () => {
    ...
    return (
      <Component
        ...
        onRetryFailed={['character_design', 'reference_generation', 'video_generation'].includes(activeStage) ? () => handleRetryFailed(activeStage) : undefined}
  ...
  return (
    <div ref={workflowRef} className="xyq-workflow">
      <TopBar
      ...
      <main className="xyq-workflow-body">
        {showHome ? (
