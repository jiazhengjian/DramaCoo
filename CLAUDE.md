...
DramaCoo is an AI video generation system that transforms user ideas into complete videos through 6 stages: Script → Character/Scene Design → Storyboard → Reference Images → Video Generation → Post-production.

This repository is DramaCoo, containing the backend (Python FastAPI) and frontend (Next.js):
...
### Backend Core
- **[orchestrator.py](backend/core/orchestrator.py)**: Workflow engine managing the 6-stage state machine. Controls session state (pending/running/waiting/completed/completed), persists to `backend/code/data/sessions/`, and coordinates agent execution.

- **[base_agent.py](backend/core/agents/base_agent.py)**: Abstract base class for all stage agents. Defines `process(input_data, intervention)` interface that returns `{"payload": ..., "requires_intervention": bool, "completed": bool}`.
...
### Tool Clients
External API integrations in `backend/models/`:
...
### Data Storage
- Results: `backend/code/result/` (image/, video/, script/)
- Session state: `backend/code/data/sessions/{session_id}.json`
